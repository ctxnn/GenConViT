...
# Import the bulletproof VAE model
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from genconvit_vae_working import GenConViTVAE
...
def train_vae_epoch(model, device, dataloader, optimizer, epoch, criterion, mse_loss):
        ...
        try:
            ...
            if batch_idx % 50 == 0:
                print(f'Train Epoch: {epoch} [{batch_idx * len(data)}/{len(dataloader.dataset)} '
                      ...
                      f'Cls: {loss_dict["cls_loss"].item():.6f}\t'
    ...
    print(f'  Reconstruction Loss: {epoch_recon_loss:.6f}')
    print(f'  KL Loss: {epoch_kl_loss:.6f}')
    print(f'  Classification Loss: {epoch_cls_loss:.6f}')
...
def train_model(dir_path, mod, num_epochs, pretrained_model_filename, test_model, batch_size):
    ...
    if mod == "ed":
        ...
        print("Using GenConViTVAE model (Bulletproof Version)")
...
def main():
    print("🚀 GenConViT Training Script - Bulletproof Edition")
    print("=" * 60)
